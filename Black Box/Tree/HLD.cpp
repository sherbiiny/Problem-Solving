...
// up: moving up in the tree (reversed in the flat tree)
// down: moving down (normal)
// each segment is {u to v} (nodes not positions
array<vector<array<int, 2>>, 2> getSortedPath(int u, int v) {
    int a = head[u], b = head[v], movingUp = 1;
    vector<array<int, 2>> up, down;
    while (a != b) {
        if (depth[a] < depth[b]) swap(a, b), swap(u, v), movingUp ^= 1;
        if (movingUp) up.push_back({u, a});
        else down.push_back({a, u});
        u = par[a], a = head[u];
    }

    if (pos[u] > pos[v]) swap(u, v), movingUp ^= 1;

    if (!movingUp) up.push_back({v, u});
    else down.push_back({u, v});
